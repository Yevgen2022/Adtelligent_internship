...
function getAdUnitSize(
  event: PrebidBidWonEvent | PrebidBidResponseEvent,
): string | undefined {
...
// bidWon - winer
export function mapBidWonEvent(
  event: PrebidBidWonEvent,
): AnalyticsEventPayload {
  return {
...
// auctionInit - auction start
export function mapAuctionInitEvent(
  event: PrebidAuctionInitEvent,
): AnalyticsEventPayload {
  return {
    event_id: generateEventId(),
...
// bidRequested - request to bider
export function mapBidRequestedEvent(
  event: PrebidBidRequestedEvent,
): AnalyticsEventPayload[] {
  return event.bids.map((bid) => ({
    ...
    user_agent:
      typeof navigator !== "undefined" ? navigator.userAgent : undefined,
    ad_unit_size: bid.sizes?.[0]
      ? `${bid.sizes[0][0]}x${bid.sizes[0][1]}`
      : undefined,
    device_type: getDeviceType(),
    browser: getBrowser(),
    os: getOS(),
...
// bidResponse - response from bider
export function mapBidResponseEvent(
  event: PrebidBidResponseEvent,
): AnalyticsEventPayload {
  return {
    event_id: generateEventId(),
...
// auctionEnd - auction's finish
export function mapAuctionEndEvent(
  event: PrebidAuctionEndEvent,
): AnalyticsEventPayload {
  ...
  if (event.bidsReceived && event.bidsReceived.length > 0) {
    const cpms = event.bidsReceived.map((bid: BidLike) => bid.cpm ?? 0);
